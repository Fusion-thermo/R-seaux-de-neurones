...
workbook = xlrd.open_workbook(r"C:/Users/jeanb/OneDrive/Documents/Python/Algo génétiques et réseaux de neurones/Raisin_Dataset/Raisin_Dataset - scaled2.xls")
...
liste_X=[]
liste_Y=[]
total=[sheet.row_values(i, start_colx=0, end_colx=None) for i in range(1,sheet.nrows)]
random.shuffle(total)

for data in total:
	liste_X.append(data[:-2])
	liste_Y.append(data[-2:])
...
def evaluation():
    ...
    for i in range(len(X_train)):
        ...
        sortie_attendue = np.argmax(np.array([Y_train[i]])[0])
        sortie_predite = np.argmax(modele.predict(entree,verbose=3)[0])
        # print(sortie_attendue,np.array([Y_train[i]])[0])
        # print(sortie_predite,modele.predict(entree,verbose=3)[0][0])
        # print('/')
        # print(entree)
        # print(modele.predict(entree,verbose=0))
        # print(modele.predict(entree,verbose=0)[0])
        # print(modele.predict(entree,verbose=0)[0][0])
        if sortie_predite == sortie_attendue:
            nb_correct += 1
    ...
    for i in range(len(X_test)):
        ...
        sortie_attendue = np.argmax(np.array([Y_test[i]])[0])
        sortie_predite = np.argmax(modele.predict(entree,verbose=3)[0])
        # print(entree)
